...
# En la siguiente sección se utilizan las funciones creadas
# en la configuración para el modelo BPSK. Con ciertas modificaciones,
# las cuales son prudentes segun la teoría para esta nueva modulación.
...
def modulador_16QAM(bits, fc, mpp):
    ...
    # 2. Matriz reordenada a partir del vector unidemensional
    # "bits".
    matriz_nueva_bits = bits.reshape(-1, 4) #Matriz NX4 con el método reshape. 
    # Donde (-1) es la dimensión desconocida.

    # 3. Construyendo un periodo de la señal portadora s(t)
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp*2)  # mpp: muestras por período por cada 2 bits
    portadorasin = np.sin(2*np.pi*fc*t_periodo) # Señal seno que compone la portadora s(t).
    portadoracos = np.cos(2*np.pi*fc*t_periodo) # Señal coseno que compone la portadora s(t).

    # 4. Inicializar la señal modulada s(t)
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx = np.zeros(t_simulacion.shape)
    moduladora = np.zeros(t_simulacion.shape)  # Señal moduladora de bits
 
    # 5. Asignar las formas de onda según los bits (16-QAM).
    '''
    Se eligen las amplitudes de acuerdo a las combinaciones de los 
    bits en la matriz de la siguiente manera: 
    ...
    i=0 # Se inicializa el contador como sigue:
    for b1, b2, b3, b4 in matriz_nueva_bits:
        if b1 == 0 and b2 == 0:
        ...
        if b3 == 1 and b4 == 0:
            senal_Tx[(i+2)*mpp : (i+4)*mpp] = portadorasin * -3
            moduladora[(i+2)*mpp : (i+4)*mpp] = -3
        i=i+4 # La iteración se da a pasos de 4 a la hora de recorrer la matriz.

    portadora = portadorasin + portadoracos # Aporte de ambas ondas ortogonales para la construcción de portadora.
    
    # 6. Calcular la potencia promedio de la señal modulada
    P_senal_Tx = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
...
def demodulador_16QAM(senal_Rx, portadorasin, portadoracos, mpp):
    '''Un método que simula un bloque demodulador
    ...
    # Demodulación:
    for i in range(0, N, 4): # Posee un paso de 4 por cada iteración. 
        ...
        # El número que corresponde al límite superior e inferior de de la energía [-40, 40]
        # es el único valor que genera una cantidad de 0 errores, por lo tanto supone los 
...
def bits_a_rgb_16QAM(bits_Rx, dimensiones):
    '''Un blque que decodifica el los bits
    recuperados en el proceso de demodulación
...
print("\n\nModulación 16-QAM:")
print("(Imagenes enviadas y recuperadas con las formas de onda obtenidas).\n")

# En la siguiente sección se verifica la congruencia entre la 
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadorasin, portadoracos, moduladora = modulador_16QAM(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso_16QAM(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador_16QAM(senal_Rx, portadorasin, portadoracos, mpp)

# 6. Se visualiza la imagen recibida 
imagen_Rx = bits_a_rgb_16QAM(bits_Rx, dimensiones)
Fig = plt.figure(figsize=(10,6))
...
# %%
# 4.2 Comprobación de estacionaridad y ergodicidad:
...
print("\n\nErgodicidad de senal_Tx:\n")

# La ergodicidad se define como la igualdad entre el promedio estadístico 
# y el promedio temporal de un proceso aleatorio.

# Por otro lado, para verificar la estacionaridad se debe comprobar que
# las variables estadísticas no cambian transitoriamente. 

# 1. Cálculo del promedio estadístico de senal_Tx

# Ahora bien, se debe obtener el promedio estadístico de los datos 
# de la señal transmitida usando la función "average" de numpy. 
estadistico_senal_Tx = np.average(senal_Tx)

print("Promedio estadístico de la señal transmitida (senal_Tx): {:0.5f}".format(estadistico_senal_Tx))

# 2. Cálculo del promedio temporal de senal_Tx.
...
# Periodo en segundos que demuestra continuidad en la función

t_simulacion = np.linspace(0, Ns*Tc, Ns*mpp)

T = t_simulacion[len(t_simulacion)-1]

# Utilizando el operador para integrar con el fin de obtener el 
# el promedio temporal:
temporal_senal_Tx = (1 / T) * np.trapz(senal_Tx, t_simulacion)

print("Promedio temporal de la señal transmitida (senal_Tx): {:0.5f}".format(temporal_senal_Tx))

# 3. Cálculo de porcentaje de error entre el promedio estadístico y temporal
err = (abs(temporal_senal_Tx-estadistico_senal_Tx)/temporal_senal_Tx)*100

print("Porcentaje de error entre promedios: {:0.5f}%".format(err))

# De ahí que, si se cumple a grandes rasgos una igualdad (porque va a existir error) 
# entre estos va a ser un proceso ergódico. 

print("\nDebido a que el porcentaje de error es pequeño, se cumple que ambos promedios son aproximadamente iguales y por tanto ergódicos.")

print("Además se comprueba estacionaridad porque en sí misma la ergodicidad es una forma restrictiva de estacionaridad y por tanto sus propiedades estadísticas no varían con el tiempo. Lo cual se puede comprobar mediante su criterio:\n\n")

mediadatos_senal_Tx = np.mean(senal_Tx)

mediatemporal_senal_Tx = np.mean(temporal_senal_Tx)

print("E[X(t)] = {:0.6f} = CONSTANTE, lo cual es señal de su estacionaridad. \n\n".format(mediatemporal_senal_Tx) )

#------------------------------------------------------------------------------------------------------------------------------------------------
print("En la siguiente gráfica se puede visualizar la gráfica de una señal X(t) con respecto al tiempo de un proceso 16-QAM. \n\n")

# Se genera la gráfica de las realizaciones de un proceso aleatorio, 
# las mismas señalan la gráfica de X(t) con respectoto al tiempo de 
# un proceso 16-QAM. 
...
# Formas de onda posibles, como es 16-QAM, se pueden esperar 16
# ondas en el tiempo. 
X_t = np.empty((8, len(t_muestra)))	   
...
# Promedio para cada punto en estado transitorio:
...
plt.plot(t_muestra, E, '-.', lw=3,color='r',label='Valor teórico')

# Mostrar las realizaciones, y su promedio calculado y teórico
plt.title('Realizaciones del proceso aleatorio $X(t)$')
plt.xlabel('$Tiempo (s)$')
plt.ylabel('Funciones dependientes del tiempo $X_i (t)$')
plt.legend()
plt.show()
...
# Gráfica
print("\n\nDensidad Espectral de Potencia de la senal_Tx:\n")
plt.title("Densidad Espectral de Potencia de senal_Tx")
plt.xlabel("Frecuencia (Hz)")
plt.ylabel("Magnitud")
plt.plot(f, 2.0/Nm * np.power(np.abs(senal_f[0:Nm//2]), 2),color='g')
plt.xlim(0, 20000)
plt.grid()
plt.show()
